...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.save_wav_b = Button(self, text = "Save to WAV", command = self.save_to_wav)
        self.save_wav_b.grid(row = 11, column = 1, columnspan = 2, sticky = W+E)

        # Sample rate selection for WAV export
        self.samplerate_l = Label(self, width = 15, text = "WAV Sample Rate").grid(row = 12, column = 0, sticky = W)
        self.samplerate_var = StringVar()
        self.sr_44_r = Radiobutton(self, text = "44.1 kHz", variable = self.samplerate_var, value = "44100")
        self.sr_44_r.grid(row = 12, column = 1, sticky = W)
        self.sr_48_r = Radiobutton(self, text = "48 kHz", variable = self.samplerate_var, value = "48000")
        self.sr_48_r.grid(row = 12, column = 2, sticky = W)
        self.sr_96_r = Radiobutton(self, text = "96 kHz", variable = self.samplerate_var, value = "96000")
        self.sr_96_r.grid(row = 12, column = 3, sticky = W)
        self.sr_48_r.invoke()  # Default to 48 kHz
    ...
    def save_to_wav(self):
        """Build DSC call and save to WAV file"""
        try:
            ...
            # Validate MMSI
            if len(a_mmsi) != 9:
                messagebox.showerror("Error", "To MMSI must be 9 digits")
                return
            if len(s_mmsi) != 9:
                messagebox.showerror("Error", "Self MMSI must be 9 digits")
                return
            ...
            # Build DSC call based on format type
            if fmt_symbol == 102:  # Area
                a_symbol = area_symbol(area)
                if len(dfreq) > 12:
                    messagebox.showerror("Error", "Frequency data too long")
                    return
            ...
            # Display the call symbols
            self.dsc_label.set(dsc_call)

            # Ask user for filename
            default_filename = f"dsc_{s_mmsi}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.wav"
            ...
            if filename:
                ...
                # Save to WAV file
                saved_file = save_dsc_to_wav(dsc_call, pwr, filename, sample_rate)

                # Show success message with details
                sr_khz = sample_rate / 1000.0
                messagebox.showinfo("Success", f"DSC signal saved to:\n{saved_file}\n\nSample Rate: {sr_khz} kHz")
