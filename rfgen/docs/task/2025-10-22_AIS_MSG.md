...
### 5. ⚠️ **КРИТИЧЕСКОЕ #1:** Исправление пути к IQ файлам (fileout)
**Проблема:** Неправильная директория для сохранения IQ файлов
...
### 6. ⚠️ **КРИТИЧЕСКОЕ #2:** Исправление передачи Path в run_loop (hackrf)
**Проблема:** `'str' object has no attribute 'exists'` при Start Tx для HackRF backend
**Причина:**
- `HackRF.run_loop()` ожидает `Path` объект (строка 122: `iq_path: Path`)
- Все UI страницы передавали `str(temp_path)` вместо `temp_path`
- В `backends/hackrf.py` строка 155 вызывается `iq_path.exists()` → ошибка!

**Исправлено в:**
- `page_ais.py` - 2 вызова (loop + repeat)
- `page_121.py` - 2 вызова (loop + repeat)
- `page_dsc_hf.py` - 2 вызова (loop + repeat)
- `page_dsc_vhf.py` - 1 вызов
- `page_navtex.py` - 2 вызова (loop + repeat)

```python
# Было (НЕПРАВИЛЬНО):
self._hackrf_backend.run_loop(
    str(temp_path),  # ← строка вместо Path!
    ...
)

# Стало (ПРАВИЛЬНО):
self._hackrf_backend.run_loop(
    temp_path,  # ← Path объект
    ...
)
```
**Результат:** ✅ HackRF backend теперь корректно работает, ошибка "'str' object has no attribute 'exists'" полностью устранена

**Примечание:** `page_406.py` и `page_gen_basic.py` уже передавали Path правильно, не требовали исправлений

### 7. Исправление режима Finite (repeat)
**Проблема:** В режиме Finite (Frame Count = 5, Gap = 5s) передача велась непрерывно как Loop
**Причина:**
- В `HackRF.run_loop()` параметр `mode` по умолчанию = `"loop"`
- UI страницы НЕ передавали параметр `mode` при вызове `run_loop()`
- Даже для режима "repeat" использовался дефолт → флаг `-R` → бесконечный цикл!

**Как работало неправильно:**
1. UI создаёт файл с `(frame + gap) * N` повторений (например, 5 кадров с паузами)
2. UI вызывает `run_loop()` БЕЗ параметра `mode`
3. Backend видит `mode="loop"` (дефолт) → включает флаг `-R` → передаёт непрерывно!
4. Результат: 5 кадров повторяются бесконечно вместо однократной передачи

**Исправлено в:**
- `page_ais.py` - добавлен if/else для передачи mode
- `page_121.py` - добавлен if/else для передачи mode
- `page_dsc_hf.py` - добавлен if/else для передачи mode
- `page_dsc_vhf.py` - добавлен if/else для передачи mode
- `page_navtex.py` - добавлен if/else для передачи mode

```python
# Loop mode:
self._hackrf_backend.run_loop(
    temp_path, ...,
    mode="loop"  # Явно указываем loop
)

# Repeat mode (ИСПРАВЛЕНО):
self._hackrf_backend.run_loop(
    temp_path, ...,
    mode="repeat",  # ← Передаём repeat вместо дефолта!
    repeat=1,       # Файл уже содержит N повторений
    gap_s=0.0       # Gap уже встроен в файл
)
```

**Результат:**
✅ **Loop mode**: работает с флагом `-R` (бесконечная передача)
✅ **Finite mode**: передаёт файл 1 раз БЕЗ `-R` (5 кадров + паузы, затем СТОП)
✅ После окончания передачи HackRF останавливается автоматически

**Примечание:** `page_406.py` использует другую логику - передаёт 1 кадр в backend, который сам делает конкатенацию `(frame+gap)*N`. Исправления не требовал.
...
- `ad44ac1` - ⚠️ Критическое #1: Исправлена ошибка пути IQ файлов (profiles_dir → out_dir)
- `ca7b4e4` - Обновлена документация AIS: добавлены все исправления
- `be02e13` - ⚠️ Критическое #2: Исправлена передача Path в run_loop (str(temp_path) → temp_path)
- `a39c209` - Документация: добавлено второе критическое исправление
- `6245917` - Исправлен режим Finite: добавлена передача параметра mode в run_loop
