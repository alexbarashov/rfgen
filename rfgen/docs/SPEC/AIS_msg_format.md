...
* **Частота дискретизации (рекомендуемая).** **Fs ≥ 1.024 МS/с** для захвата без потерь, **SPS (samples per symbol) ≈ 106.7** при Fs=1.024e6
...
7. **Передискретизация (upsampling):**
   ...
   - Пример: Fs=1.024e6, Rs=9600 → SPS≈106.7 → округлить до 106
