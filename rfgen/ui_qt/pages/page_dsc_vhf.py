...
class PageDSC_VHF(QWidget):
    ...
    def _load_default_profile(self):
        """Auto-load default_dsc_vhf.json profile if it exists on startup."""
        default_path = profiles_dir() / "default_dsc_vhf.json"
    ...
    def _start_fileout(self, prof):
        ...
        try:
            # Generate IQ
            self.status_label.setText("Generating DSC VHF signal...")
            self.update()

            iq = build_dsc_vhf(prof)
    ...
    def _start_hackrf(self, prof):
        """Generate and transmit DSC VHF via HackRF."""
        from ...standards.dsc_vhf import build_dsc_vhf
        ...
        try:
            ...
            iq_baseband = build_dsc_vhf(prof)

            # 2. No resample needed (DSC generates directly at TX rate)
            iq_tx = iq_baseband
            fs_tx = prof["device"]["fs_tx"]

            # 3. Handle schedule mode
            ...
            temp_filename = generate_cf32_name(fs_tx, "temp_dsc_vhf", add_timestamp=False)
            ...
            self.status_label.setText(
                f"TX: 156.525 MHz (Ch70), Fs={fs_tx/1e6:.1f} MS/s, "
